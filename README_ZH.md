...
| C++ 项目嵌入 Node.js                 | 嵌入式 | Node.js 所有       | ⚠️ 部分支持 | jspp 借用 `isolate` 和 `context` <br> `EsModule` **特性不支持(Node.js 已接管 v8 模块解析回调)** <br/> **libnode** 动态库需要导出 v8 符号 |
| Node.js Addon                        | 扩展   | Node.js 所有       | ❌ 未测试   | 暂未实现 `ESModule` 机制                                                                                                                 |
| QuickJS 原生模块                     | 扩展   | 宿主所有           | ❌ 未测试   | 暂未实现 `ESModule` 机制                                                                                                                 |
| QuickJS 原生模块(宿主使用 jspp)      | 扩展   | 宿主的 jspp 所有   | ❌ 未测试   | 暂未实现 `ESModule` 机制, 潜在的 Opaque 冲突                                                                                             |
| QuickJS 原生模块(宿主使用其它绑定库) | 扩展   | 宿主所有           | ❌ 未测试   | 暂未实现 `ESModule` 机制, 潜在的 Opaque 冲突                                                                                             |
